...
When you use the Terraform resource aws_iam_user_login_profile with password_length = 20, Terraform sends the request to AWS. AWS creates the password, but then hides it.
To see that password in your Terraform output, you usually have to use PGP Encryption (a complex method involving public/private keys). If you don't want to deal with that technical headache, There are two alternatives:
AWS doesn't return auto-generated passwords without PGP encryption. To set passwords:

### Option 1: AWS Console (use if you only have 1 to 5 users.)
...
### Option 2: AWS CLI (use if you have 6 to 20 users.)

```powershell
aws iam create-login-profile --user-name mscott --password "TempPassword123!" --password-reset-required
```

### Option 3: Use PGP in terraform (if you have 50+ users)
