# 22: RDS Database (Mini Project 8)
...
- `modules/security_groups`: Configures AWS Security Groups. One for the web server allows public HTTP (port 80) and SSH (port 22) access. Another for the database restricts MySQL (port 3306) access only to the web server's security group, enforcing a secure perimeter.
- `modules/ec2`: Launches an AWS EC2 instance (acting as the web server) in a public subnet. It's configured to establish a connection to the RDS database using the database endpoint, username, and password passed from other modules. This module likely uses a user_data.sh script (found in modules/ec2/templates/) to automate web server setup and database client configuration upon launch.
