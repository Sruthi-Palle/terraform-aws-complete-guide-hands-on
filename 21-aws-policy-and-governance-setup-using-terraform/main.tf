  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        #If someone tries to talk to the bucket OR any file inside it without using HTTPS (SSL), block them immediately." 
        #If you forgot the /*, someone might be blocked from listing the bucket via HTTP, but they might still be able to download a file if they knew the direct link!
        Resource = [
