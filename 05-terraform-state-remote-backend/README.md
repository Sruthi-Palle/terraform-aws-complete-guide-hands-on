...
- **Example:** If your [`main.tf`](http://main.tf) defines an S3 bucket named "my-app-bucket", after `terraform apply`, the `terraform.tfstate` file will contain an entry for that bucket, including its actual ARN, creation date, and other attributes.
...
- **Drift Detection:** It helps Terraform detect "drift" – situations where manual changes have been made to your infrastructure outside of Terraform.  
...
You should use a multi-folder structure that keeps your foundational backend resources isolated from your application logic.

In 2026, we take advantage of **Native S3 State Locking** (introduced in Terraform 1.10), which means we no longer need a DynamoDB table, making your setup much cleaner.
...
This creates the "Home" for all your future state files.

Navigate to `00-bootstrap/` and create the following [`main.tf`](http://main.tf). Note that we **initially** have no `backend` block.  
**File:** `00-bootstrap/`[`main.tf`](http://main.tf)
...
2. **Result:** Your bucket is created. Your state is currently a local `terraform.tfstate` file in this folder.

### Step 2: The Bootstrap Migration (The "Inception" Step)

Now, add this block to the **top** of the same `00-bootstrap/`[`main.tf`](http://main.tf) file:
...
2. **Result:** Terraform will ask to copy local state to S3. Type `yes`. Now the bootstrap folder is self-managed in the cloud.

**Implementation Checklist**

1. **Init & Apply:** Run in `00-bootstrap` without the backend block first.
2. **Migrate:** Uncomment the backend block and run `terraform init -migrate-state`.
3. **Verify Locking:** Check the S3 bucket settings in the AWS Console. Under the "Objects" tab, look for a `terraform.tfstate.tflock` metadata file once you start running operations—this confirms **Native S3 Locking** is active.
   - `.terraform.lock.hcl` → **provider dependency lock file (LOCAL)**
   - `terraform.tfstate.tflock` → **S3 object used for execution locking**

4. **Reference:** In your `01-project-app`, remember to use the `data "terraform_remote_state"` block if you need to pull any IDs (like VPC IDs) from the foundation.
...
Creating a separate [`backend.tf`](http://backend.tf) keeps your provider settings clean and separated from your resource logic.

**File:** `01-project-app/`[`backend.tf`](http://backend.tf)
...
Now you can write your `resource` code for creating VPC or any other resource in `01-project-app/`[`main.tf`](http://main.tf) file.

**File:** `01-project-app/`[`main.tf`](http://main.tf)
...
- **Backend Connection:** Terraform reads [`backend.tf`](http://backend.tf), connects to your S3 bucket, and locates the `project-app/terraform.tfstate` path.
- **Plugin Download:** It downloads the necessary AWS provider versions.
- **Locking Check:** Because `use_lockfile = true` is set, Terraform verifies it has permission to create and delete `.tflock` objects in that S3 path.
...
- **Comparison:** Terraform compares your [`main.tf`](http://main.tf) (the VPC) against the current state (which is currently empty) and proposes creating 1 resource.
...
1. **Re-Acquiring Lock:** Terraform locks the state again to ensure no changes happen during the build.
2. **Resource Creation:** Terraform calls the AWS APIs to create your VPC.
3. **State Write:** Once AWS confirms the VPC is created, Terraform writes the metadata (VPC ID, ARN, etc.) into the S3 bucket.
4. **Final Release:** The `.tflock` file is deleted from S3.

---

### 3.3. How to Verify the 2026 Native Locking

To prove that your "no-DynamoDB" setup is working, you can check two things:
...
1. **Impenetrable Safety:** If a developer accidentally runs `terraform destroy` in the `project-app` folder, it is **physically impossible** for the S3 bucket to be deleted, because that bucket is managed by a totally different state file in the `bootstrap` folder.
2. **No DynamoDB Overhead:** By using `use_lockfile = true`, you eliminate the cost and complexity of a DynamoDB table.
3. **Clean Audit Trail:** Every environment (Dev, Prod, Bootstrap) has its own clear path (`/key`) in S3.
...
In your `01-project-app/`[`backend.tf`](http://backend.tf), you chose the path `project-app/terraform.tfstate`. As your infrastructure grows, you should split this further to minimize the **Blast Radius**.
...
> **Why?** If you make a mistake in your "Compute" code and corrupt the state, your "Network" state remains untouched and safe.

---

### Using `terraform_remote_state` (Cross-Folder Data)

Since your `01-project-app` is now in a separate folder from your `00-bootstrap` (or other future folders), how do you get data from one to the other? Use the `terraform_remote_state` data source.
...
- **Purpose:** Provides commands for direct manipulation of the Terraform state file.
- **Subcommands:**
  - `terraform state list`: Lists all resources in the current state.
  - `terraform state show resource_type.resource_name`: Displays details of a specific resource in the state.
  - `terraform state mv old_address new_address`: Moves a resource within the state file (e.g., if you refactor your HCL).
  - `terraform state rm resource_type.resource_name`: Removes a resource from the state file _without destroying it_ in the cloud. **Extremely dangerous if not understood fully.**
  - `terraform state pull`: Downloads the current remote state file to local output.
  - `terraform state push path/to/local.tfstate`: Uploads a local state file to the remote backend.
