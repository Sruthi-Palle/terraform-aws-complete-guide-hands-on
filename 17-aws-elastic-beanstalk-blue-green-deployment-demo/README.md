...
Using **AWS Elastic Beanstalk** to achieve zero-downtime deployments through blue-green deployment strategy.
Elastic Beanstalk creates your application, launches an environment, makes an application version, then deploys your code into the environment.
...
| **Setting**    | **Why?**                                                                             |
| -------------- | ------------------------------------------------------------------------------------ |
| **IAM**        | Without `IamInstanceProfile`, the server can't download your code.                   |
| **Networking** | If you want a Load Balancer, you must set `EnvironmentType` to `LoadBalanced`.       |
| **Capacity**   | You must tell it the `InstanceType` (e.g., `t3.micro`) so you don't get a huge bill. |
| **Updates**    | You need `DeploymentPolicy` so your site doesn't go down during an update.           |
