...
Managing and provisioning computing infrastructure (e.g., networks, virtual machines, load balancers) using configuration files typically stored in version control.
...
- `terraform refresh` (or refresh during plan/apply)
...
- **Provisioning vs. Configuration:** Terraform and CloudFormation are primarily for provisioning infrastructure. Ansible, Chef, and Puppet are primarily for configuration management on existing servers.
