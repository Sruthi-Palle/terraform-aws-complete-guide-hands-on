...
- Note: I have alreaded added ssh.tf file for creating SSH key pairs while running terraform apply command itself. If you want to create them using aws cli, you can follow below approach
...
## SSH key pair generation best practices

### For Development/Testing/Learning (current use case):

- Using the Terraform `tls` provider is a great choice. The convenience and full automation it offers are perfect for environments that are frequently created and destroyed. The security risk of the key in the state file is often considered acceptable for non-production scenarios.

### For Production Environments:

- Neither of the above is considered the absolute "best practice". The recommended approach for production is to use a dedicated secrets management system like AWS Secrets Manager or HashiCorp Vault.
- In that workflow, you would securely store your pre-generated private key in the secrets manager.
- Terraform would then use a data source (e.g., data "aws_secretsmanager_secret_version") to fetch the key material at apply time, without ever storing it in the state file.
- This gives you the best of both worlds: the security of managing secrets separately and the automation of a full IaC workflow.
